## --------------- HEADER ------------------------------------------------------
## Script name: 2d_Dormancy-class-colonization-extinction-model.R
...
# Model colonization with quasibinomial
# col.ever.m2 <- glm(Colonized.ever ~ DormancyClass * Treatment,
# data = dormancy.col, family = quasibinomial(link = "logit")
# )
...
write.csv(col.model.coef, "Animals-plants-seeds/Analysis/Plants/Colonization-coef.csv",
  row.names = FALSE
)

col.model.summ <- glance(col.ever.m)
write.csv(col.model.summ, "Animals-plants-seeds/Analysis/Plants/Colonization-summ.csv",
  row.names = FALSE
)

write.csv(col.ever.means, "Animals-plants-seeds/Analysis/Plants/Colonization-means.csv",
  row.names = FALSE
)
...
write.csv(ext.ever.model.coef, "Animals-plants-seeds/Analysis/Plants/Extinction-coef.csv",
  row.names = FALSE
)

ext.ever.model.summ <- glance(ext.ever.m)
write.csv(ext.ever.model.summ, "Animals-plants-seeds/Analysis/Plants/Extinction-summ.csv",
  row.names = FALSE
)

write.csv(ext.ever.means, "Animals-plants-seeds/Analysis/Plants/Extinction-means.csv",
  row.names = FALSE
)
